...
void dijkstra()
{
    ...
    for (int i = 1; i < N; i++)
    {
        cout<<graphTRANS[src]<<"->"<<graphTRANS[i]<<"\t\t"<<dist[i]<<"\t\t"<<graphTRANS[src]<<" ";
        printPath(i);
        cout<<endl;
    }
}
...
printLSAAndNeighbors();
	 std::cout << "***********Converted globalID!!!!!!********" << "\n";
	std::cout << globalMyID << "\n";
	 std::cout << convertedSource << "\n";
	std::cout << graphTRANS[graphTranslationINDEX] << "\n";
...
map<string, string>::iterator it;    
    ...
    //formattedString.append(it->first);
     ...
     stringstream convertedWeight(it->second);
              ...
              std::cout << it->first << "\n";
               ...
               std::cout << graphTRANSIN[it->first] << "\n";
        u = 0;
    ...
    //std::cout << "PRINTING*************LSA FOR NODE***************" << "\n";
        ...
        graphTranslationINDEX++;
        int sourceIndex = graphTranslationINDEX;
        
        graphTRANSIN[iterator->first] = graphTranslationINDEX;
        graphTRANS[graphTranslationINDEX] = iterator->first;
...
string sourceToParse = p->first;


std::string SOURCE = sourceToParse.substr(0, sourceToParse.find(delimiter));

std::cout << "***********PARSED SOURCE********" << "\n";

               std::cout << SOURCE << "\n";

sourceToParse.erase(0, sourceToParse.find(delimiter) + delimiter.length());

std::string COST = sourceToParse.substr(0, sourceToParse.find(delimiter));


std::cout << "***********PARSED COST********" << "\n";

               std::cout << COST << "\n";

std::cout << "***********GRAPH TRANS source*******" << "\n";

               std::cout << graphTranslationINDEX << "\n";

std::cout << "***********GRAPH TRANS destination*******" << "\n";

               std::cout <<   graphTRANSIN[SOURCE] << "\n";

//== m.end()

        if (graphTRANSIN.find(SOURCE) == graphTRANSIN.end()){
std::cout << "***********No TRANSLATION EXISTS!!! Creating one. *******" << "\n";

      graphTranslationINDEX++;
        
        graphTRANSIN[SOURCE] = graphTranslationINDEX;
        graphTRANS[graphTranslationINDEX] = SOURCE;


 stringstream sourceWeight(COST);
...
COST.erase(std::remove(COST.begin(), COST.end(), '|'), COST.end());
